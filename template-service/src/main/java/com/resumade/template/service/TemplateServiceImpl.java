...
import com.resumade.template.exception.TemplateNotFoundException;
import com.resumade.template.repository.TemplateRepository;

@Service
// Manages template lifecycle: seed data, CRUD, usage tracking, and admin access control
public class TemplateServiceImpl implements TemplateService {

    private final TemplateRepository templateRepository;

        // Default fallback HTML/CSS when template creation omits layout markup
    ...
    // Seeds 3 built-in templates on first startup (empty DB) — idempotent via count check
    ...
    public List<TemplateResponse> getFreeTemplates() {
        return templateRepository.findByIsActiveTrueAndIsPremiumFalse().stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public List<TemplateResponse> getPremiumTemplates() {
        return templateRepository.findByIsActiveTrueAndIsPremiumTrue().stream()
                .map(this::mapToResponse)
                .toList();
    }

    @Override
    public List<TemplateResponse> getTemplatesByCategory(String categoryStr) {
        Template.Category category = Template.Category.valueOf(categoryStr.toUpperCase());
        return templateRepository.findByIsActiveTrueAndCategory(category).stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public TemplateResponse getTemplateById(Integer id) {
        return mapToResponse(findTemplateOrThrow(id));
    }

    @Override
    @Transactional
    public TemplateResponse createTemplate(TemplateRequest request, String role) {
        checkAdminAccess(role);
        Template template = new Template();
        mapRequestToTemplate(request, template);
        return mapToResponse(templateRepository.save(template));
    }

    @Override
    @Transactional
    public TemplateResponse updateTemplate(Integer id, TemplateRequest request, String role) {
        checkAdminAccess(role);
    ...
    private Template findTemplateOrThrow(Integer id) {
    ...
    private void mapRequestToTemplate(TemplateRequest request, Template template) {
        template.setName(request.getName());
        template.setDescription(request.getDescription());
        template.setThumbnailUrl(request.getThumbnailUrl());
        template.setHtmlLayout(normalizeOrDefault(request.getHtmlLayout(), DEFAULT_HTML_LAYOUT));
        template.setCssStyles(normalizeOrDefault(request.getCssStyles(), DEFAULT_CSS_STYLES));
        ...
        if (request.getHasPhoto() != null) {
            template.setHasPhoto(request.getHasPhoto());
        } else if (template.getHasPhoto() == null) {
            template.setHasPhoto(false);
        }

        if (request.getHasSkillBars() != null) {
            template.setHasSkillBars(request.getHasSkillBars());
        } else if (template.getHasSkillBars() == null) {
            template.setHasSkillBars(false);
        }
    ...
    private TemplateResponse mapToResponse(Template template) {
        return TemplateResponse.builder()
                ...
                .hasPhoto(template.getHasPhoto())
                .hasSkillBars(template.getHasSkillBars())
