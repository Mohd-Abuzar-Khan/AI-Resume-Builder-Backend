...
@Service
// AI orchestration layer — wraps Gemini API with retry, failover, and JSON
...
public class AiServiceImpl implements AiService {
    ...
    public AtsReport checkAtsCompatibility(Integer userId, Integer resumeId, String resumeContent,
        ...
        log.error("ATS Check failed after {} retries: {}", maxRetries + 1,
    ...
    public List<String> suggestSkills(String jobTitle) {
        String prompt = String.format(
                "Suggest the top 10 most important technical skills for a %s. Return only a comma-separated list of skills.",
                jobTitle);
        try {
    ...
    @Override
    public String generateCoverLetter(Integer userId, Integer resumeId, String resumeContent, String jobDescription) {
        String prompt = String.format(
                """
                        You are an expert cover letter writer. Your task is to generate a professional, persuasive, and highly personalized cover letter based on the candidate's resume and the target job description.

                        ═══════════════════════════════════════════
                        STEP 1 — EXTRACT FROM RESUME (do this silently before writing)
                        ═══════════════════════════════════════════
                        Extract the following fields from the resume. If a field is missing, use the placeholder shown:
                        - Full Name              → [Candidate Name]
                        - Phone Number           → [Phone]
                        - Email Address          → [Email]
                        - City / Address         → [City, State]
                        - LinkedIn / Portfolio   → (omit line if missing)
                        - Current or Most Recent Job Title → [Your Title]
                        - Years of Experience    → infer from work history
                        - Top 3 relevant skills for THIS job → use in body paragraphs

                        ═══════════════════════════════════════════
                        STEP 2 — GENERATE THE COVER LETTER
                        ═══════════════════════════════════════════
                        Use the structure below EXACTLY. Do not add section headers or labels in the output.

                        [Full Name]
                        [City, State] · [Phone] · [Email]
                        [LinkedIn URL or Portfolio — only if present in resume]

                        [Today's Date]

                        [Hiring Manager Name, or "Hiring Manager" if unknown]
                        [Company Name, inferred from job description]
                        [Company Location, if mentioned]

                        Dear [Hiring Manager Name or "Hiring Team"],

                        OPENING PARAGRAPH (2–3 sentences):
                        - State the exact role being applied for.
                        - Lead with a compelling hook — a specific achievement from the resume or a strong value statement.
                        - Convey genuine enthusiasm for this company/role specifically.

                        BODY PARAGRAPH 1 (3–4 sentences):
                        - Highlight 1–2 of the candidate's most relevant experiences that directly match requirements in the job description.
                        - Use specific numbers, outcomes, or technologies from the resume wherever possible.
                        - Mirror keywords from the job description naturally (for ATS compatibility).

                        BODY PARAGRAPH 2 (3–4 sentences):
                        - Address a second requirement or challenge from the job description.
                        - Connect the candidate's skills or projects to the company's goals or domain.
                        - Mention a specific tool, stack, or methodology from the resume that aligns with the JD.

                        CLOSING PARAGRAPH (2–3 sentences):
                        - Express eagerness for an interview.
                        - Include a soft call to action ("I would love the opportunity to discuss...").
                        ...
                        ═══════════════════════════════════════════
                        - Word count: 300–400 words (body only, excluding header/signature).
                        - Tone: Professional, confident, and enthusiastic — never generic or robotic.
                        - DATA ADHERENCE: Use ONLY the experiences, achievements, and metrics found in the RESUME CONTENT provided below.
                        - NO HALLUCINATIONS: Do not mention technologies, companies, or degrees that are not in the resume.
                        - TAILORING: Connect the candidate's existing achievements to the needs mentioned in the JOB DESCRIPTION.
                        - Do NOT repeat the resume verbatim — synthesize and explain why that experience matters for THIS specific role.
                        - Output ONLY the cover letter. No explanations, no commentary.
    ...
    // Executes AI call with audit trail — persists request/response to DB for usage
    // tracking
    ...
    private String callGeminiWithModel(String model, String promptText, double temperature) {
        // WARNING: API key visible in URL query string — appears in logs and proxy
        // caches
    ...
    // Multi-stage JSON cleanup pipeline: strip markdown fences → extract JSON →
    // remove trailing commas → fix truncation → balance brackets
    ...
    private String sanitizeTruncatedJson(String text) {
        ...
        // Check if the JSON is already balanced (extractBalancedJson would have
        // returned it)
        ...
        // Valid boundaries: after a closing brace/bracket, after a quoted string, after
        // a number, after true/false/null
        while (sb.length() > 1) {
            ...
            if (Character.isDigit(last) || last == '.') {
                ...
                while (numStart > 0 && (Character.isDigit(sb.charAt(numStart - 1)) || sb.charAt(numStart - 1) == '.'
                        || sb.charAt(numStart - 1) == '-')) {
                    numStart--;
                }
                ...
                if (beforeNum >= 0 && (sb.charAt(beforeNum) == ':' || sb.charAt(beforeNum) == ','
                        || sb.charAt(beforeNum) == '[')) {
            ...
            // If it's e/E (scientific notation part), a letter in true/false/null, or
        ...
        // If we end with an orphan colon (truncated after key name before value),
        // remove the key too
