...
// AI endpoint controller — uses untyped Map<String, Object> payloads instead of typed DTOs
public class AiController {

    private static final String RESUME_CONTENT_KEY = "resumeContent";
    ...
    public ResponseEntity<AtsReport> checkAts(
        ...
        AtsReport report = aiService.checkAtsCompatibility(
                userId,
                extractInt(payload, "resumeId"),
                (String) payload.getOrDefault(RESUME_CONTENT_KEY, ""),
    ...
    public ResponseEntity<String> generateCoverLetter(
        ...
        String result = aiService.generateCoverLetter(
                userId,
                extractInt(payload, "resumeId"),
                (String) payload.get(RESUME_CONTENT_KEY),
                (String) payload.get("jobDescription"));
        return ResponseEntity.ok(result);
    }
    ...
    // WARNING: Allows arbitrary prompt execution with no auth gate — should be @Profile("dev") or admin-only
    @PostMapping("/test")
    ...
    // Extracts userId set by JwtAuthFilter — returns null if filter was bypassed (public path)
